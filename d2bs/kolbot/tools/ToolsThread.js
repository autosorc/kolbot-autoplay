...
function main() {
	var i, mercHP, ironGolem, tick, merc, hideChickens,
	...
	this.gameEvent = function (mode, param1, param2, name1, name2) {
		...
		case 0x12: // "Diablo Walks the Earth"
			...
			if (Config.StopOnDClone && me.gametype === 1) { // only do this in expansion
				...
				} else if (Hunter.profile && !Hunter.isHunter) {
					print("ÿc4Hunterÿc0: " + Hunter.profile + " started");
					Hunter.needHelp = true;
					me.maxgametime = (Hunter.settings.maxWait * 1000 * 60) + (getTickCount() - me.gamestarttime);
					load("tools/idle.js");
					D2Bot.start(Hunter.profile);
				}
			}
