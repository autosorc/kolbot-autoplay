...
var D2Bot = {
	...
	init: function () {
		...
		if (!s || !s.running) {
			if (FileTools.exists("tools/iDB.js")) {
				load("tools/iDB.js");
			}
		}*/
...
MainLoop:
		while (true) {
			...
			case 8: // main menu
				if (info.realm) {
					ControlAction.clickRealm(realms[info.realm.toLowerCase()]);
				}
		...
		if ((getLocation() === 12 || getLocation() === 42) && me.realm.toLowerCase() !== info.realm.toLowerCase()) {
			D2Bot.restart();
		}
				...
				do {
					...
					if (text instanceof Array && typeof text[1] === "string") {
						...
						if (text[1].toLowerCase() === info.charName.toLowerCase()) {
							if (text.indexOf("EXPANSION CHARACTER") === -1) {
