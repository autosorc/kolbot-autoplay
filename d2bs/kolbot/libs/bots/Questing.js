...
function Questing() {
	...
	this.killIzual = function () {
		...
		sendPacket(1, 0x31, 4, tyrael.gid, 4, 684);
		delay(500);
		sendPacket(1, 0x38, 4, 0, 4, tyrael.gid, 4, 109);
	...
	if (me.gametype == 1 && !Pather.accessToAct(5) && Pather.accessToAct(4) && !!me.getQuest(26, 0)) {
		Town.goToTown(4);
		var tyrael = getUnit(1, NPC.Tyrael);
	...
	D2Bot.printToConsole("All quests done");
	
	if (getScript("D2BotAutoCreate.dbj")) {
		scriptBroadcast(["Normal","Nightmare","Hell"][me.diff]);
	} else {
		D2Bot.stop();
	}
