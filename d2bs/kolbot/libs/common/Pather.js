...
ModeLoop:
			...
			do {
				if ((getDistance(door, x, y) < 4 && getDistance(me, door) < 9) || getDistance(me, door) < 4) {
					for (i = 0; i < 3; i += 1) {
						...
						if (me.classid === 1 && me.getSkill(43, 1) && i < 3) {
	...
	*/
	moveToPreset: function (area, unitType, unitId, offX, offY, clearPath, pop) {
		...
		if (!presetUnit) {
			throw new Error("moveToPreset: Couldn't find preset unit - area: " + area + " unitId: " + unitId);
		}
	...
	*/
	...
	/*
		...
		targetArea - area id of where the unit leads to
	*/
	useUnit: function (type, id, targetArea, clearPath) {
		var i, tick, unit, coord,
			preArea = me.area;
		...
		for (i = 0; i < 5; i += 1) {
			...
			if (getDistance(me, unit) > 5) {
				this.moveToUnit(unit, 0, 0, clearPath);
			}
...
MainLoop:
	...
	talkTo: function (npc, townloc, menuid) {
		...
		if (!unit || !unit.openMenu()) return false;
		if (menuid) return Misc.useMenu(menuid);
		me.cancel();
	...
	changeAct: function(act) {
		...
		if (npcUnit) {
			for (let i = 0; i < 5; i += 1) {
				sendPacket(1, 0x38, 4, 0, 4, npcUnit.gid, 4, areaID);
				delay(500 + me.ping);
...
MainLoop:
	...
	*/
	journeyTo: function (area, clearPath=false) {
