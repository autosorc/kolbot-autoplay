	...
	checkItem: function (unit) {
		...
		if ((NTIP.GetMercTier(unit) > 0 || NTIP.GetTier(unit) > 0 || NTIP.GetSecondaryTier(unit) > 0) && unit.getFlag(0x10)) {
			...
			if (Item.autoEquipCheckSecondary(unit)) {
				return {
					result: 1,
					line: "Equip secondary tier: " + NTIP.GetSecondaryTier(unit)
				};
			}
		...
		if (rval.result === 0 && !getBaseStat("items", unit.classid, "quest") && Town.ignoredItemTypes.indexOf(unit.itemType) === -1 && unit.itemType !== 39 && (unit.location === 3 || me.gold < Config.LowGold)) {
			...
			if (unit.location === 3 || unit.getItemCost(1) / (unit.sizex * unit.sizey) >= Config.MinGold) {
				return {
					result: 4,
					line: "Gold value: " + unit.getItemCost(1)
				};
			}
		}
		...
		while (pickList.length > 0) {
			...
			if (copyUnit(pickList[0]).x !== undefined && (pickList[0].mode === 3 || pickList[0].mode === 5) &&
				...
				if (status.result && this.canPick(pickList[0])) {
					...
					if (!canFit && Config.FieldID) {
						if (!Attack.getNearestMonster(15) && Town.fieldID()) {
							canFit = Storage.Inventory.CanFit(pickList[0]) || [4, 22, 76, 77, 78].indexOf(pickList[0].itemType) > -1;
						}
					}
