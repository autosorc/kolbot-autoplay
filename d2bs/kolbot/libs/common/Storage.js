...
var Container = function (name, width, height, location) {
	...
	/* Container.SortItems()
	 *	Sorts items in a container to one side
	 */
...
/*    this.SortItems = function() { 
        ...
        for (y = this.width - 1; y >= 0; y--) {
            for (x = this.height - 1 ; x >= 0; x--) {
				...
				if (moved.indexOf(item.gid) > -1) continue;
	...
	this.cubeSpot = function() {
		...
		if (cube.location === 7 && cube.x === 0 && cube.y === 0) {
			return true;
		}
	...
	this.SortBySize = function (a, b) { //Sort by size > x > y
		return b.sizex * b.sizey - a.sizex * a.sizey || b.x - a.x || b.y - a.y;         
	};
	...
	this.SortItems = function() { 
		...
		Storage.Reload();
		items = me.findItems(-1, -1, this.location).sort(this.SortBySize);
		...
		for (i = 0; i < items.length; i++) {
			...
			if (this.location === 3 && this.IsLocked(items[i], Config.Inventory)) {
                continue;
            }
			
			pos = this.FindSpot(items[i]);
			...
			if (pos.y > items[i].y && pos.x === items[i].x) {
				continue;
			}				

			if (pos.x === items[i].x && pos.y === items[i].y) {
				continue;
			}
...
Loop:
			for (x = 0; x < this.height - (item.sizey - 1); x += 1) {
				//If there's an item on this spot AND (the item is not the one we're trying to move OR the item isn't in the same location)
				if (this.buffer[x][y] > 0 && (this.location !== item.location || this.itemList[this.buffer[x][y] - 1].gid !== item.gid)) {
					continue;
				}
				...
				for (nx = 0; nx < item.sizey; nx += 1) {
					for (ny = 0; ny < item.sizex; ny += 1) {
						if (this.buffer[x + nx][y + ny] && (this.location !== item.location || this.itemList[this.buffer[x + nx][y + ny] - 1].gid !== item.gid)) {
							continue Loop;
						}
					}
				}
