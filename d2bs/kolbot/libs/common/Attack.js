...
var Attack = {
	...
	kill: function (classId) {
		...
		let maxAttack = Config.MaxAttackCount;
		...
		while (attackCount < maxAttack && this.checkMonster(target) && this.skipCheck(target)) {
			Misc.townCheck();
	...
	// Clear array of coords
	clearCoordList: function (list, pick) {
		for (let node of list) {
			this.clear(node.radius);
	...
	clearLevel: function (spectype, nodeDistance) {
		if (Config.MFLeader) {
			Pather.makePortal();
		...
		while (rooms.length > 0) {
			...
			result = Pather.getNearestWalkable(room[0], room[1], nodeDistance, 3);
	...
	// Get nearest monster from self
	getNearestMonster: function (range=30) {
		var gid, distance,
			monster = getUnit(1);

		if (monster) {
			do {
				if (this.checkMonster(monster) && !monster.getParent() && this.canAttack(monster)) {
