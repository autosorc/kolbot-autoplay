...
Unit.prototype.drop = function () {
	if (this.type !== 4) {
		throw new Error("Unit.drop: Must be used with items. " + this.toSource());
	}
...
me.findItem = function (id, mode, loc, quality, bodyloc) {
	if (id === undefined) {
		id = -1;
	}
	...
	if (item) {
		do {
			if ((loc === -1 || item.location === loc) && (quality === -1 || item.quality === quality) && (bodyloc === -1 || item.bodylocation === bodyloc)) {
				return item;
			}
		} while (item.getNext());
	}
